...
//VAO_START ADD
// Hack for making the server start always in sharedApp mode
    windowLocation = @"0,0";
//VAO_END
...
- (void) finish_select:(CGPoint)loc{
    windowLocation = [[NSString stringWithFormat:@"%f,%f", loc.x, loc.y] retain];
    NSLog(@"LOG:__%@", windowLocation);
    if (cover_window) {
...
//VAO_START ADD
    if(windowLocation){
        [argv addObject:@"-sharedwin"];
        [argv addObject:windowLocation];
    }
//VAO_END
