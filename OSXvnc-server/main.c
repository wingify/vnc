...
//VAO_BY
//Bool rfbLocalhostOnly = TRUE;
//Bool rfbInhibitEvents = TRUE;
//VAO_END
...
//VAO_START
//// cl->requestedRegion being nonempty is considered an indication of a client request
//// being pending. So, if we set it, we make the server think that client has made a
//// request. This is wrong, and hence we move this code to the place (a few lines below)
//// where we have first ensured that the client has indeed made a request.
//            // GRW - SharedAppVnc
...
//VAO_START
// here, we have ensured that the client has indeed made a request.
                // GRW - SharedAppVnc
                if ([sharedApp enabled] && cl->supportsSharedAppEncoding)
                {
                    //get windows rects and set to cl->requestedRegion
                    ...
                    // since an empty &cl->requestedRegion means there is no request pending, make sure it does not get emptied by us.
                    BOOL isNonEmpty = REGION_NOTEMPTY(&hackScreen, &cl->requestedRegion);
                    if(!isNonEmpty){
                        BoxRec box;
                        box.x1 = 0;
                        box.y1 = 0;
...
//VAO_START
// This function `rfbStartClientWithFD` was a part of `listenerRun` in original code.
...
//VAO_START REPLACE
						exit(1);
//VAO_BY
////`listen_fd` has been renamed to `listen_fd4`
//                        // GRW - SharedAppVnc
//                        close(listen_fd4);
//                        pthread_exit(NULL);
//VAO_END
...
//VAO_START REPLACE
// In v4.01, a declaration of this function has also been added in rfb.h,
// which was not there in v1.6. So, we need to change there also.
//void connectReverseClient(char *hostName, int portNum) {
//VAO_BY
// GRW modified
rfbClientPtr connectReverseClient(char* hostName, int portNum) {
//VAO_END
    pthread_t client_thread;
    rfbClientPtr cl;
...
static void processArguments(int argc, char *argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "-rfbport") == 0) { // -rfbport port
            ...
            rfbPort = atoi(argv[++i]);
//VAO_START ADD
        } else if (strcmp(argv[i], "-sharedwin") == 0) { // shared window
            if (i + 1 >= argc) usage();
            char *arg_x = argv[++i];
            char *arg_y = arg_x;
            while(*arg_y++ != ',');
...
//VAO_START REPLACE
//allow scanning for a broader range of ports
	//while (tryPort < 5910) {
//VAO_BY
	while (tryPort < 5940) {
//VAO_END
...
//VAO_START ADD
// initialize sharedApp before calling processArguments as processArguments will need sharedApp
    [[SharedApp alloc] init]; // this sets the global variable.
//VAO_END
