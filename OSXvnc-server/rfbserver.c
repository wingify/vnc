...
// The portion of code to revert to IPv4 (since IPv6 wasn't working) is not merged
// because OSXvnc_4.01 has that fixed in it's own code
//VAO_END
    int sock= -1;
...
//VAO_START
#ifdef SHAREDAPP
	cl->supportsSharedAppEncoding = TRUE; //FALSE;
#endif
//VAO_END
...
// edited for VAO
			...
			if (/*msg.type == sharedAppPointerEvent && */[sharedApp enabled])
			{
				...
				windowId = (msg.type == sharedAppPointerEvent) ? Swap32IfLE(msg.spe.windowId) : 0;
				win = [sharedApp getWindowWithId:windowId];
				...
				if (win)
				{
                    ...
                    if(msg.type == rfbPointerEvent){
                        CGPoint winOrigin = [win location].origin;
                        x += winOrigin.x;
                        y += winOrigin.y;
                        msg.pe.x = Swap16IfLE(x);
                        msg.pe.y = Swap16IfLE(y);
                    }
...
Bool rfbSendUpdateBuf(rfbClientPtr cl) {
    ...
    if (WriteExact(cl, cl->updateBuf, cl->ublen) < 0) {
        ...
        NSLog(@"%@",[NSThread callStackSymbols]); // print stack trace
        rfbCloseClient(cl);
        return FALSE;
    }
